// Package marketing implements TikTok API for Business Marketing API v1.3
// for one advertiser account. Paid advertising resources remain separate from
// the organic TikTok adapter and are not mapped onto common social posts.
package marketing
...
const (
	adapterName      = "tiktok/business-marketing-api-v1.3"
	platformName     = "tiktok"
	productName      = "business-marketing-api"
	apiVersion       = "v1.3"
	defaultBaseURL   = "https://business-api.tiktok.com/open_api"
	defaultAuthURL   = "https://ads.tiktok.com"
...
// intended for tests, TikTok sandbox, and private HTTP gateways.
type Settings struct {
	BaseURL              string `json:"base_url,omitempty" yaml:"base_url,omitempty"`
	AuthorizationBaseURL string `json:"authorization_base_url,omitempty" yaml:"authorization_base_url,omitempty"`
}

// AccountSettings binds a social-hub account to one TikTok advertiser.
type AccountSettings struct {
	AdvertiserID string `json:"advertiser_id" yaml:"advertiser_id"`
}

// Adapter implements socialhub.Adapter for TikTok Business Marketing API.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be business-marketing-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if !validID(typed.AdvertiserID) {
			return invalidArgument("init", "account.settings.advertiser_id must be a numeric string")
		}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	tokens := socialhub.StaticTokenSource{Value: socialhub.Token{AccessToken: accessToken, TokenType: "TikTokBusiness"}}
...
// OAuth returns helpers for Marketing API browser authorization and long-term
// token exchange. Marketing long-term tokens do not have a refresh token.
func (adapter *Adapter) OAuth(ctx context.Context, accountID socialhub.AccountID) (*OAuthClient, error) {
	...
	if !validID(account.AppID) || strings.TrimSpace(account.SecretRef) == "" {
		return nil, invalidArgument("oauth", "app_id and secret_ref are required")
	}
	...
	httpClient.CheckRedirect = rejectRedirect
	return &OAuthClient{
		AppID: account.AppID, Secret: secret, BaseURL: settings.BaseURL,
		AuthorizationBaseURL: settings.AuthorizationBaseURL, HTTPClient: &httpClient,
	}, nil
}
