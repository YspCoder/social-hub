// Package marketing implements ad-account-scoped Snapchat Marketing API v1
// workflows. Paid-media resources remain separate from common social posts.
package marketing
...
const (
	adapterName      = "snapchat/marketing-api-v1"
	platformName     = "snapchat"
	productName      = "marketing-api"
	apiVersion       = "v1"
	defaultBaseURL   = "https://adsapi.snapchat.com/v1"
	defaultAuthURL   = "https://accounts.snapchat.com/login/oauth2/authorize"
	defaultTokenURL  = "https://accounts.snapchat.com/login/oauth2/access_token"
	documentationURL = "https://developers.snap.com/marketing-api/Ads-API/"
	marketingScope   = "snapchat-marketing-api"
)

// Settings controls Snapchat API and OAuth endpoints. Overrides are intended
...
// AccountSettings binds one social-hub account to one Snapchat Ad Account.
type AccountSettings struct {
	AdAccountID string `json:"ad_account_id" yaml:"ad_account_id"`
}

// Adapter implements socialhub.Adapter for Snapchat Marketing API v1.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Metadata() socialhub.Metadata {
	return socialhub.Metadata{
		...
		DocURL: documentationURL + "introduction", VerifiedAt: time.Date(2026, time.August, 9, 0, 0, 0, 0, time.UTC),
	}
}
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	for _, endpoint := range []string{settings.BaseURL, settings.AuthURL, settings.TokenURL} {
		if !validEndpoint(endpoint) {
			return invalidArgument("init", "Snapchat endpoints must be absolute HTTP(S) URLs without credentials, query, or fragment")
		}
	}
	for _, account := range config.Accounts {
		if strings.TrimSpace(account.AccessTokenRef) == "" {
			return invalidArgument("init", "access_token_ref is required for every Snapchat Marketing account")
		}
		var typed AccountSettings
		if err := socialhub.DecodeSettings(account.Settings, &typed); err != nil {
			return platformError("init", socialhub.CodeInvalidArgument, socialhub.ClassPermanent, err)
		}
		if !validUUID(typed.AdAccountID) {
			return invalidArgument("init", "account.settings.ad_account_id must be a UUID")
		}
...
// OAuth returns Snapchat's authorization-code and refresh-token helper.
