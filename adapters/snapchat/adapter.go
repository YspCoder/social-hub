// Package snapchat implements Snapchat Public Profile API v1.
package snapchat
...
const (
	adapterName     = "snapchat/public-profile-v1"
	apiVersion      = "v1"
	defaultAPIURL   = "https://businessapi.snapchat.com"
	...
	docURL          = "https://developers.snap.com/marketing-api/Public-Profile-API/GetStarted"
	requiredScope   = "snapchat-profile-api"
)

// CapabilityPublicProfileWorkflow identifies Snapchat's typed profile and
// Spotlight read workflow.
const CapabilityPublicProfileWorkflow socialhub.Capability = "public_profile_workflow"

// Settings controls Snapchat Public Profile API and OAuth endpoints.
...
// AccountSettings identifies the Public Profile authorized by the token.
type AccountSettings struct {
	ProfileID string `json:"profile_id" yaml:"profile_id"`
}

// Adapter implements socialhub.Adapter for Snapchat Public Profile API v1.
type Adapter struct {
	mu       sync.RWMutex
...
func (a *Adapter) Metadata() socialhub.Metadata {
	return socialhub.Metadata{
		Name: adapterName, Product: "snapchat-public-profile", APIVersion: apiVersion,
...
func (a *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	for _, endpoint := range []string{settings.BaseURL, settings.AuthURL, settings.TokenURL} {
		if !validEndpoint(endpoint) {
			return invalidArgument("init", "all Snapchat endpoints must be absolute HTTP(S) URLs")
		}
	}
	for _, account := range config.Accounts {
		...
		if err := socialhub.DecodeSettings(account.Settings, &typed); err != nil || strings.TrimSpace(typed.ProfileID) == "" {
			return invalidArgument("init", "account.settings.profile_id is required")
		}
	}
...
func (a *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	httpTransport, err := transport.New(settings.BaseURL, resolved.HTTPClient, tokens, "snapchat", "snapchat-public-profile", decodeHTTPError)
	if err != nil {
		return nil, platformError("client", socialhub.CodeInvalidArgument, socialhub.ClassPermanent, err)
	}
	client := &Client{
		accountID: accountID, profileID: typed.ProfileID, transport: httpTransport,
		scopes: append([]string(nil), account.Approval.Scopes...),
	}
	client.profiles = &PublicProfileService{client: client}
	return client, nil
}

// OAuth returns a Snapchat OAuth 2.0 helper for one configured app.
