// Package baiduads implements the unversioned Baidu Marketing API contract
// verified in August 2026. Paid advertising resources remain behind typed
...
const (
	adapterName      = "baidu/marketing-api-2026-08"
	platformName     = "baidu"
	productName      = "marketing-api"
	apiVersion       = "unversioned-2026-08"
	defaultBaseURL   = "https://api.baidu.com"
	defaultOAuthURL  = "https://u.baidu.com"
	documentationURL = "https://dev2.baidu.com/content"
...
// AccountSettings binds a social-hub account to one Baidu Ads username.
type AccountSettings struct {
	UserName string `json:"user_name" yaml:"user_name"`
}

// Adapter implements socialhub.Adapter for Baidu Marketing API.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	settings := Settings{BaseURL: defaultBaseURL, OAuthBaseURL: defaultOAuthURL}
	...
	for _, account := range config.Accounts {
		...
		if !validUserName(typed.UserName) {
			return invalidArgument("init", "account.settings.user_name is required")
		}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	authenticator := transport.AuthenticatorFunc(func(*http.Request, socialhub.Token) error { return nil })
	tokens := socialhub.StaticTokenSource{Value: socialhub.Token{AccessToken: accessToken, TokenType: "BaiduAds"}}
	...
	return &Client{accountID: accountID, userName: typed.UserName, accessToken: accessToken, api: api}, nil
}

// OAuth returns helpers for Baidu's authorization-code and refresh-token flows.
func (adapter *Adapter) OAuth(ctx context.Context, accountID socialhub.AccountID) (*OAuthClient, error) {
	...
	if !validOpaque(account.AppID, 128) || strings.TrimSpace(account.SecretRef) == "" {
	...
	httpClient.CheckRedirect = rejectRedirect
	return &OAuthClient{
		AppID: account.AppID, SecretKey: secret, BaseURL: settings.OAuthBaseURL,
