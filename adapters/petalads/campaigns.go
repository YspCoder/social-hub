...
func (client *Client) ListCampaigns(ctx context.Context, input ListCampaignsRequest, options ...socialhub.CallOption) (CampaignPage, error) {
	...
	// Huawei's documented wire contract uses a JSON request body on this GET.
	...
	if err != nil || len(response.Campaigns) > input.PageSize || total < len(response.Campaigns) {
		return CampaignPage{}, platformContractError(operation, "Petal Ads returned invalid Campaign pagination")
	}
	for _, campaign := range response.Campaigns {
		if !validCampaign(campaign) {
			return CampaignPage{}, platformContractError(operation, "Petal Ads returned an invalid Campaign")
		}
	}
	return CampaignPage{
		Campaigns: response.Campaigns, Page: input.Page, PageSize: input.PageSize, Total: total,
		HasMore: input.Page <= total/input.PageSize && input.Page*input.PageSize < total,
	}, nil
}
...
func validCampaign(campaign Campaign) bool {
	return validID(campaign.ID) && validResponseText(campaign.Name, 512) && validResponseText(campaign.Status, 128) &&
		...
		validReportValue(campaign.TodayDailyBudget) && validReportValue(campaign.TomorrowDailyBudget) &&
		(campaign.CreatedTime == "" || validTimestamp(campaign.CreatedTime)) && validResponseText(campaign.Type, 128) &&
		validResponseText(campaign.MarketingGoal, 128) && validResponseText(campaign.FlowResource, 128) &&
		validResponseText(campaign.SyncFlowResourceSearchAds, 128)
}
