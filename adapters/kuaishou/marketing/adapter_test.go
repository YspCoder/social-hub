...
func TestAdapterMetadataCapabilitiesAndLifecycle(t *testing.T) {
	...
	if !slices.Contains(socialhub.Adapters(), adapterName) || adapter.Name() != adapterName || metadata.APIVersion != apiVersion || metadata.VerifiedAt.IsZero() {
	...
	if client.Platform() != platformName || client.Account() != "ads-primary" || client.Advertisers() == nil ||
		client.Campaigns() == nil || client.Units() == nil || client.Creatives() == nil || client.Reports() == nil {
