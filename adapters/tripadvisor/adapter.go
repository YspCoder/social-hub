// Package tripadvisor implements the read-only Tripadvisor Content API v1.
package tripadvisor
...
const (
	adapterName      = "tripadvisor/content-api-v1"
	platformName     = "tripadvisor"
	productName      = "content-api"
	apiVersion       = "1.0.0"
	defaultBaseURL   = "https://api.content.tripadvisor.com/api/v1"
	documentationURL = "https://tripadvisor-content-api.readme.io/reference/overview"
)

// Adapter implements socialhub.Adapter for Tripadvisor Content API v1.
type Adapter struct {
	mu      sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be content-api")
	}
	...
	if len(config.Settings) > 0 {
		return invalidArgument("init", "adapter settings are not supported; Content API uses the fixed official HTTPS origin")
	}
	for _, account := range config.Accounts {
		if !validOpaque(account.AccessTokenRef, 4096) {
			return invalidArgument("init", "account.access_token_ref for the Tripadvisor API key is required")
		}
		if account.ClientID != "" || account.AppID != "" || account.SecretRef != "" || account.TokenStore != "" {
			return invalidArgument("init", "client_id, app_id, secret_ref, and token_store are not used with a Tripadvisor API key")
		}
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "webhook settings are not used by Content API reads")
		}
		if account.Approval.AccountType != "" || len(account.Approval.Scopes) > 0 {
			return invalidArgument("init", "OAuth account type and scopes are not used with a Tripadvisor API key")
		}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	api, err := transport.NewWithAuthenticator(
		...
		platformName, productName, transport.QueryAuthenticator("key"), newHTTPErrorDecoder(resolved.Clock, apiKey),
	)
	if err != nil {
