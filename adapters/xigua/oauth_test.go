...
func TestOAuthAuthorizationExchangeRefreshAndClientToken(t *testing.T) {
	...
	oauthServer := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/oauth/access_token/":
			...
			_, _ = writer.Write([]byte(`{"data":{"access_token":"user-token-1","refresh_token":"refresh-1","open_id":"open-id-1","expires_in":"1296000","refresh_expires_in":"2592000","scope":"user_info,xigua.video.data","error_code":"0"}}`))
	...
	authorizationURL, err := oauth.AuthorizationURL("https://app.example/callback?tenant=1", "state-1", []string{"user_info", "xigua.video.data"})
	if err != nil {
		t.Fatal(err)
	}
	parsed, err := url.Parse(authorizationURL)
	if err != nil || parsed.Host != strings.TrimPrefix(oauthServer.URL, "http://") || parsed.Path != "/oauth/connect" || parsed.Query().Get("client_key") != "client-key" || parsed.Query().Get("response_type") != "code" || parsed.Query().Get("scope") != "user_info,xigua.video.data" || parsed.Query().Get("state") != "state-1" {
		t.Fatalf("authorization URL=%q err=%v", authorizationURL, err)
	}
	userToken, err := oauth.Exchange(context.Background(), "code-1")
	if err != nil || userToken.OpenID != "open-id-1" || userToken.Token.AccessToken != "user-token-1" || userToken.Token.TokenType != "XiguaUser" || len(userToken.Token.Scopes) != 2 {
	...
	if err != nil || clientToken.AccessToken != "client-token-1" || clientToken.TokenType != "XiguaClient" || !clientToken.ExpiresAt.Equal(testNow.Add(2*time.Hour)) {
...
func TestOAuthValidationAndResponseLimits(t *testing.T) {
	...
	valid := &OAuthClient{ClientKey: "key", ClientSecret: "secret", AuthURL: "https://example.com/oauth/connect", TokenBaseURL: "https://example.com", HTTPClient: http.DefaultClient, Clock: clock}
