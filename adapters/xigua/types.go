...
func mapUser(accountID socialhub.AccountID, input userData) *socialhub.User {
	...
	return &socialhub.User{
		...
		Extensions: map[string]json.RawMessage{"xigua.user": extension},
	}
}

func mapVideo(accountID socialhub.AccountID, openID string, input xiguaVideo, observedAt time.Time) *socialhub.Post {
	...
	if input.VideoID != "" {
		media = []socialhub.Media{{
			...
			Extensions: map[string]json.RawMessage{"xigua.video": mediaExtension},
		}}
	}
	metrics := []socialhub.Metric{
		{Name: "digg", Value: float64(input.Statistics.DiggCount), AsOf: observedAt, Definition: "Xigua statistics.digg_count"},
		{Name: "comments", Value: float64(input.Statistics.CommentCount), AsOf: observedAt, Definition: "Xigua statistics.comment_count"},
		{Name: "plays", Value: float64(input.Statistics.PlayCount), AsOf: observedAt, Definition: "Xigua statistics.play_count"},
		{Name: "shares", Value: float64(input.Statistics.ShareCount), AsOf: observedAt, Definition: "Xigua statistics.share_count"},
		{Name: "forwards", Value: float64(input.Statistics.ForwardCount), AsOf: observedAt, Definition: "Xigua statistics.forward_count"},
	}
	return &socialhub.Post{
		...
		Media: media, Extensions: map[string]json.RawMessage{"xigua.video": mediaExtension},
