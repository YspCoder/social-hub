...
func TestAdapterRegistrationMetadataCapabilitiesAndClose(t *testing.T) {
	...
	if err != nil || !capabilities[CapabilityAdManager].Supported || capabilities[socialhub.CapFetch].Supported {
		t.Fatalf("capabilities=%#v err=%v", capabilities, err)
	}
	if client.Platform() != platformName || client.Account() != testAccountID || client.Inventory() != client || client.Delivery() != client || client.Reporting() != client {
...
func TestAdapterInitValidation(t *testing.T) {
	...
	tests := []func(socialhub.AdapterConfig) socialhub.AdapterConfig{
		...
		func(value socialhub.AdapterConfig) socialhub.AdapterConfig {
			value.Accounts[0].Settings = map[string]any{"network_code": "bad"}
			return value
		},
		func(value socialhub.AdapterConfig) socialhub.AdapterConfig {
...
func TestAdapterClientAndOAuthErrors(t *testing.T) {
	server := httptest.NewServer(http.NotFoundHandler())
	defer server.Close()
	adapter := &Adapter{}
	if err := adapter.Init(context.Background(), staticConfig(server.URL),
		socialhub.WithHTTPClient(server.Client()), socialhub.WithSecretResolver(mapResolver{"test://client-secret": "client-secret"}),
	); err != nil {
		t.Fatal(err)
	}
...
func TestReadOnlyScopeGatesWriteBeforeNetwork(t *testing.T) {
	requestSeen := false
	...
	config.Accounts[0].Approval.Scopes = []string{readOnlyScope}
	adapter := &Adapter{}
	if err := adapter.Init(context.Background(), config, socialhub.WithHTTPClient(server.Client()), socialhub.WithSecretResolver(mapResolver{"test://access-token": testAccessToken, "test://client-secret": "secret"})); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := client.CreateHiddenReport(context.Background(), CreateReportRequest{DisplayName: "Read only", Definition: reportDefinition()}); !errors.Is(err, socialhub.ErrApprovalRequired) || requestSeen {
...
func TestRedirectsAreRejected(t *testing.T) {
	redirected := false
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.URL.Path == "/v1/"+networkName() {
	...
	if _, err := client.GetNetwork(context.Background()); err == nil || redirected {
