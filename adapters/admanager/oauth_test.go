...
func TestOAuthAuthorizationExchangeAndRefresh(t *testing.T) {
	...
	query := parsed.Query()
	if query.Get("client_id") != "client-id" || query.Get("scope") != fullScope || query.Get("state") != "state-value" || query.Get("access_type") != "offline" || query.Get("prompt") != "consent" {
...
func TestManagedRefreshTokenSourceCachesAndRefreshes(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/token":
			...
			writeJSON(t, writer, http.StatusOK, map[string]any{"access_token": "managed-" + string(rune('0'+call)), "expires_in": 3600, "scope": fullScope, "token_type": "Bearer"})
		case "/v1/" + networkName():
	...
	if _, err := client.GetNetwork(context.Background()); err != nil {
		t.Fatal(err)
	}
	if _, err := client.GetNetwork(context.Background()); err != nil || tokenCalls.Load() != 1 {
		t.Fatalf("cached request err=%v tokenCalls=%d", err, tokenCalls.Load())
	}
	clock.Set(testNow.Add(2 * time.Hour))
	if _, err := client.GetNetwork(context.Background()); err != nil || tokenCalls.Load() != 2 {
...
func TestOAuthErrorsAndValidation(t *testing.T) {
	...
	if validCallbackURL("ftp://example.com/callback") || validCallbackURL("https://user:pass@example.com") || !validCallbackURL("http://localhost/callback") ||
		validOAuthScopes(nil) || validOAuthScopes([]string{"openid"}) || validOAuthScopes([]string{fullScope, fullScope}) || boundedMessage(strings.Repeat("x", 30), 20) != strings.Repeat("x", 20) {
