...
func (client *Client) ListCampaigns(ctx context.Context, input CampaignSelector, options ...socialhub.CallOption) (Page[Campaign], error) {
	...
	return postPage(ctx, client, operation, campaignServicePath+"/get", request, input.PageRequest,
		MaximumPageSize, campaignEntity, func(value *Campaign) error {
...
func (client *Client) CreateCampaign(ctx context.Context, input CampaignAdd, options ...socialhub.CallOption) (*Campaign, MutationResult[Campaign], error) {
	...
	if !validCampaignAdd(input) {
		return nil, MutationResult[Campaign]{}, invalidArgument(operation, "standard CPC campaign name, budget, or dates are invalid")
	}
	...
	operand := Campaign{
		CampaignName: input.Name, Type: CampaignStandard, UserStatus: StatusPaused,
		Budget: &CampaignBudget{Amount: &budget},
		BiddingStrategyConfiguration: &CampaignBiddingStrategy{
			BiddingScheme: &CampaignBiddingScheme{BiddingStrategyType: BiddingCPC},
		},
	...
	if created == nil || created.UserStatus != StatusPaused || created.Type != CampaignStandard {
		return created, result, platformContractError(operation, "created campaign was not returned as a paused STANDARD campaign")
	}
...
func (client *Client) UpdateCampaigns(ctx context.Context, inputs []CampaignUpdate, options ...socialhub.CallOption) (MutationResult[Campaign], error) {
	...
	if len(inputs) == 0 || len(inputs) > MaximumMutationBatch {
		return MutationResult[Campaign]{}, invalidArgument(operation, "1-2000 campaign updates are required")
	}
...
func (client *Client) SetCampaignsEnabled(ctx context.Context, ids []int64, enabled bool, options ...socialhub.CallOption) (MutationResult[Campaign], error) {
	...
	if !validIDs(ids, MaximumMutationBatch, false) {
		return MutationResult[Campaign]{}, invalidArgument(operation, "1-2000 unique campaign IDs are required")
	}
	status := StatusPaused
	if enabled {
		status = StatusActive
...
func (client *Client) validateCampaign(operation string, value *Campaign, expectedID int64) error {
	if value == nil || value.AccountID != client.advertiserAccountID || value.CampaignID <= 0 ||
		!validText(value.CampaignName, 50) || !validOptionalDate(value.StartDate) || !validOptionalDate(value.EndDate) ||
		value.StartDate != "" && value.EndDate != "" && value.StartDate > value.EndDate ||
		(value.UserStatus != StatusActive && value.UserStatus != StatusPaused) {
	...
	if value.Type != CampaignStandard && value.Type != CampaignMobile && value.Type != CampaignDynamic {
		return platformContractError(operation, "LINE Yahoo returned an unsupported campaign type")
	}
...
func (client *Client) validateCampaignMutation(operation string, value *Campaign) error {
	if value == nil || value.CampaignID <= 0 ||
