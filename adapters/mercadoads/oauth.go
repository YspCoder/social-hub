...
// OAuthClient implements Mercado Libre's server-side authorization-code flow.
// PKCE is optional at the platform level and mandatory when enabled for the app.
...
// TokenResult preserves the Mercado Libre user bound to the token pair.
...
// AuthorizationURL builds a state-protected authorization URL. State is
// required by this SDK even though Mercado Libre only recommends it.
...
// Exchange exchanges a one-time authorization code for a user token pair.
...
// Refresh consumes one refresh token and returns the newly rotated token pair.
