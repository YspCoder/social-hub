// Package mercadoads implements Mercado Libre Product Ads read and metrics workflows.
package mercadoads
...
const (
	adapterName      = "mercadolibre/product-ads-api-v2"
	platformName     = "mercadolibre"
	productName      = "product-ads-api"
	apiVersion       = "v2"
	...
	documentationURL = "https://developers.mercadolibre.com.ar/en_us/product-ads-us-read"
	approvalType     = "product-ads-enabled"
)
...
// AccountSettings binds one SDK account to one Product Ads advertiser. A zero
// AdvertiserID permits advertiser discovery but not advertiser-scoped lists.
...
// Adapter implements socialhub.Adapter for Mercado Libre Product Ads.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be product-ads-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "Mercado Libre notifications are outside this Product Ads adapter")
		}
		...
		if !validApproval(account.Approval) {
			return invalidArgument("init", "approval must be omitted or declare product-ads-enabled with the read scope")
		}
