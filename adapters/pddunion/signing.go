...
type pddAuthenticator struct {
	clientID    string
	accessToken string
	clock       socialhub.Clock
}

func (authenticator *pddAuthenticator) Authenticate(request *http.Request, token socialhub.Token) error {
	if request.Body == nil || token.AccessToken == "" {
		return fmt.Errorf("pddunion: request body and client secret are required")
	}
	body, err := io.ReadAll(io.LimitReader(request.Body, maxRequestBytes+1))
	_ = request.Body.Close()
	if err != nil {
		return fmt.Errorf("pddunion: read request form: %w", err)
	}
	if len(body) > maxRequestBytes {
		return fmt.Errorf("pddunion: request form exceeds 1 MiB")
	}
	values, err := url.ParseQuery(string(body))
	if err != nil {
		return fmt.Errorf("pddunion: parse request form: %w", err)
	}
	values.Set("client_id", authenticator.clientID)
	values.Set("data_type", "JSON")
	values.Set("timestamp", fmt.Sprintf("%d", authenticator.clock.Now().Unix()))
	...
	if len(payload) > maxRequestBytes {
		return fmt.Errorf("pddunion: signed request form exceeds 1 MiB")
	}
