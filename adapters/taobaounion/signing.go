...
var topLocation = time.FixedZone("UTC+8", 8*60*60)
...
func (authenticator *topAuthenticator) Authenticate(request *http.Request, token socialhub.Token) error {
	if request.Body == nil || token.AccessToken == "" {
		return fmt.Errorf("taobaounion: request body and app secret are required")
	}
	body, err := io.ReadAll(io.LimitReader(request.Body, maxRequestBytes+1))
	_ = request.Body.Close()
	if err != nil {
		return fmt.Errorf("taobaounion: read request form: %w", err)
	}
	if len(body) > maxRequestBytes {
		return fmt.Errorf("taobaounion: request form exceeds 1 MiB")
	}
	values, err := url.ParseQuery(string(body))
	if err != nil {
		return fmt.Errorf("taobaounion: parse request form: %w", err)
	}
	values.Set("app_key", authenticator.appKey)
	values.Set("format", "json")
	values.Set("sign_method", "md5")
	values.Set("simplify", "true")
	values.Set("timestamp", authenticator.clock.Now().In(topLocation).Format("2006-01-02 15:04:05"))
	values.Set("v", apiVersion)
	if authenticator.session != "" {
		values.Set("session", authenticator.session)
	}
	if authenticator.partnerID != "" {
		values.Set("partner_id", authenticator.partnerID)
	}
	values.Set("sign", signMD5(values, token.AccessToken))
	payload := values.Encode()
	if len(payload) > maxRequestBytes {
		return fmt.Errorf("taobaounion: signed request form exceeds 1 MiB")
	}
