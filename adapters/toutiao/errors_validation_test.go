...
func TestFlexibleIntegerAndValidationHelpers(t *testing.T) {
	...
	if !validOptionalText("text") || validOptionalText(strings.Repeat("x", maxTextBytes+1)) || validOptionalText(string([]byte{0xff})) {
		t.Fatal("text validation failed")
	}
	if !validScopes([]string{"user_info", "toutiao.video:create"}) || validScopes(nil) || validScopes([]string{"bad scope"}) {
...
func TestOAuthErrorSanitizationAndScopeSplitting(t *testing.T) {
	...
	scopes := splitScopes("user_info,toutiao.video.data\nother")
