// Package toutiao implements the official Toutiao OpenAPI adapter.
package toutiao
...
const (
	adapterName         = "toutiao/openapi"
	platformName        = "toutiao"
	productName         = "openapi"
	apiVersion          = "continuous"
	defaultBaseURL      = "https://open.douyin.com"
	defaultOAuthBaseURL = "https://open.snssdk.com"
	documentationURL    = "https://open.douyin.com/platform/resource/docs/ability/content-management/toutiao-publish-solution"
)

// Settings controls the separate Toutiao business and OAuth origins.
...
// AccountSettings identifies the app-scoped authorized Toutiao user.
type AccountSettings struct {
	OpenID string `json:"open_id" yaml:"open_id"`
}

// Adapter implements socialhub.Adapter for Toutiao OpenAPI.
type Adapter struct {
	mu       sync.RWMutex
...
func (a *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	settings := Settings{
		...
		AuthURL: defaultOAuthBaseURL + "/oauth/authorize/",
	}
