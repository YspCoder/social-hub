...
// Client implements the public Toutiao user and video capabilities.
type Client struct {
...
func (c *Client) Capabilities(context.Context) (socialhub.Capabilities, error) {
	return socialhub.Capabilities{
		socialhub.CapPublish: capabilityState(socialhub.CapPublish, c.scopes, []string{"toutiao.video.create"}, "small-video publication is asynchronous and subject to review", "https://open.douyin.com/platform/resource/docs/openapi/video-management/toutiao/create-video/publish-video/"),
		socialhub.CapFetch:   capabilityState(socialhub.CapFetch, c.scopes, []string{"user_info", "toutiao.video.data"}, "authorized profile and owned public video data", "https://open.douyin.com/platform/resource/docs/openapi/video-management/toutiao/search-video/account-video-list/"),
		socialhub.CapMedia:   capabilityState(socialhub.CapMedia, c.scopes, []string{"toutiao.video.create"}, "direct and multipart video upload up to the documented limits", "https://open.douyin.com/platform/resource/docs/openapi/video-management/toutiao/create-video/upload-video"),
		socialhub.CapReact:   {Capability: socialhub.CapReact, Supported: false, Approval: socialhub.ApprovalUnknown, Reason: "the public Toutiao OpenAPI does not document reaction mutation or comment APIs"},
		socialhub.CapMessage: {Capability: socialhub.CapMessage, Supported: false, Approval: socialhub.ApprovalUnknown, Reason: "the public Toutiao OpenAPI does not document direct messaging"},
		socialhub.CapWebhook: {Capability: socialhub.CapWebhook, Supported: false, Approval: socialhub.ApprovalUnknown, Reason: "no signed Toutiao webhook contract is included in this adapter version"},
	}, nil
}
...
func (c *Client) Publish(ctx context.Context, input socialhub.CreatePostRequest, options ...socialhub.CallOption) (*socialhub.Post, error) {
	if err := c.requireScope("publish", "toutiao.video.create"); err != nil {
		return nil, err
	}
	...
	if input.Text != nil && !validOptionalText(*input.Text) {
		return nil, invalidArgument("publish", "text is invalid or exceeds 32 KiB")
	}
	if input.ReplyToID != nil || input.QuotePostID != nil || input.Visibility != nil {
		return nil, unsupported("publish", "reply, quote, and visibility overrides are not supported by toutiao.video.create")
	}
	body := map[string]string{"video_id": input.MediaIDs[0]}
	if input.Text != nil && *input.Text != "" {
		body["text"] = *input.Text
	}
	var response createVideoEnvelope
	if err := c.api.JSON(ctx, http.MethodPost, "/toutiao/video/create/", c.openIDQuery(), body, &response, options...); err != nil {
		return nil, err
	}
	...
	if !validOpaque(response.Data.ItemID, maxOpaqueLength) {
		return nil, invalidPlatformResponse("publish", "response omitted a valid item_id")
	}
	status := socialhub.PublishStatus{ID: response.Data.ItemID, State: socialhub.PublishStatePending, Message: "submitted for Toutiao review"}
	c.submittedMu.Lock()
	...
	return &socialhub.Post{
		...
		AuthorID: stringPointer(c.openID), Text: input.Text,
		Media:  []socialhub.Media{{ID: input.MediaIDs[0], Type: socialhub.MediaTypeVideo, State: socialhub.MediaStateProcessing}},
		Status: &status,
	}, nil
}
...
func (c *Client) DeletePost(_ context.Context, postID string, _ ...socialhub.CallOption) error {
	...
	return unsupported("delete_post", "no public Toutiao video deletion endpoint is documented")
}
...
func (c *Client) GetPost(ctx context.Context, postID string, options ...socialhub.CallOption) (*socialhub.Post, error) {
	...
	if err := c.requireScope("get_post", "toutiao.video.data"); err != nil {
		return nil, err
	}
	var response videoListEnvelope
	if err := c.api.JSON(ctx, http.MethodPost, "/toutiao/video/data/", c.openIDQuery(), map[string][]string{"item_ids": {postID}}, &response, options...); err != nil {
		return nil, err
	}
...
func (c *Client) ListPosts(ctx context.Context, input socialhub.ListPostsRequest, options ...socialhub.CallOption) (socialhub.Page[socialhub.Post], error) {
	...
	if input.StartTime != nil || input.EndTime != nil {
		return socialhub.Page[socialhub.Post]{}, unsupported("list_posts", "toutiao.video.list does not document time filters")
	}
	if err := c.requireScope("list_posts", "toutiao.video.data"); err != nil {
	...
	var response videoListEnvelope
	if err := c.api.JSON(ctx, http.MethodGet, "/toutiao/video/list/", query, nil, &response, options...); err != nil {
		return socialhub.Page[socialhub.Post]{}, err
	}
...
func (c *Client) ListComments(_ context.Context, input socialhub.ListCommentsRequest, _ ...socialhub.CallOption) (socialhub.Page[socialhub.Comment], error) {
	...
	return socialhub.Page[socialhub.Comment]{}, unsupported("list_comments", "no public Toutiao comment-list endpoint is documented")
}

func (c *Client) openIDQuery() url.Values { return url.Values{"open_id": {c.openID}} }

func validVideo(input toutiaoVideo) bool {
	return validOpaque(input.ItemID, maxOpaqueLength) && validOpaque(input.VideoID, maxOpaqueLength)
}
