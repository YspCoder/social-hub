...
func TestDirectUploadAndTypedVideoWorkflow(t *testing.T) {
	...
	apiServer := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/toutiao/video/create/":
			...
			if body["video_id"] != "video-workflow" || body["text"] != "workflow demo" {
				writer.WriteHeader(http.StatusBadRequest)
				return
			}
	...
	job, err := workflow.Publish(context.Background(), workflowSession.ID, video.PublishRequest{Description: "workflow demo"})
...
func TestMultipartUploadRequestShapeAndLayout(t *testing.T) {
	...
	apiServer := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/toutiao/video/part/complete/":
...
func TestUploadStateConflictsAndValidation(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.URL.Path != "/toutiao/video/upload/" {
			writer.WriteHeader(http.StatusNotFound)
			return
		}
		close(started)
