...
// AuthorizationURL returns the Toutiao authorization-code URL.
...
// Refresh refreshes or extends a user access token. Toutiao does not expose
// Douyin's refresh-token renewal operation, so reauthorization is eventually required.
...
func (c *OAuthClient) ClientToken(ctx context.Context) (socialhub.Token, error) {
	...
	return socialhub.Token{
		AccessToken: payload.AccessToken, TokenType: "ToutiaoClient",
...
func (c *OAuthClient) userTokenRequest(ctx context.Context, path string, form url.Values, multipartForm bool) (UserToken, error) {
	...
	return UserToken{
		Token: socialhub.Token{
			AccessToken: payload.AccessToken, RefreshToken: payload.RefreshToken, TokenType: "ToutiaoUser",
