...
func TestAdapterRegistrationMetadataCapabilitiesAndLifecycle(t *testing.T) {
	...
	if err != nil || !capabilities.Has(CapabilityAdsManagement) {
	...
	if client.Platform() != platformName || client.Account() != testAccountID || client.Advertisers() == nil ||
		client.Campaigns() == nil || client.InsertionOrders() == nil || client.LineItems() == nil {
...
func TestAdapterValidationAndFailures(t *testing.T) {
	...
	tests := []struct {
		...
		{"advertiser", func(config *socialhub.AdapterConfig) { config.Accounts[0].Settings["advertiser_id"] = "0" }},
		{"partner", func(config *socialhub.AdapterConfig) { config.Accounts[0].Settings["partner_id"] = "partner" }},
...
func TestScopeGateStaticAuthenticationAndPartnerRequirement(t *testing.T) {
	requestSeen := false
	...
	client.scopes = []string{displayVideoScope}
	if _, err := client.GetAdvertiser(context.Background()); err != nil || !requestSeen {
		t.Fatalf("request err=%v requestSeen=%v", err, requestSeen)
	}
	client.partnerID = ""
	if _, err := client.ListAdvertisers(context.Background(), ListRequest{}); !errors.Is(err, socialhub.ErrInvalidArgument) {
		t.Fatalf("partner error=%v", err)
	}
}
