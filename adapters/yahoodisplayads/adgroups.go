...
func (client *Client) ListAdGroups(ctx context.Context, input AdGroupSelector, options ...socialhub.CallOption) (Page[AdGroup], error) {
	...
	return postPage(ctx, client, operation, adGroupServicePath+"/get", request, input.PageRequest,
		MaximumAdGroupPageSize, adGroupEntity, func(value *AdGroup) error {
...
func (client *Client) GetAdGroup(ctx context.Context, campaignID, adGroupID int64, options ...socialhub.CallOption) (*AdGroup, error) {
	...
	if campaignID <= 0 || adGroupID <= 0 {
		return nil, invalidArgument(operation, "campaign and ad group IDs must be positive")
	}
	...
	if len(page.Items) != 1 {
		return nil, platformContractError(operation, "LINE Yahoo returned multiple ad groups for one ID")
	}
	if err := client.validateAdGroup(operation, &page.Items[0], campaignID, adGroupID); err != nil {
		return nil, err
	}
...
func (client *Client) CreateAdGroups(ctx context.Context, campaignID int64, inputs []AdGroupAdd, options ...socialhub.CallOption) (MutationResult[AdGroup], error) {
	...
	for _, input := range inputs {
		if !validAdGroupAdd(input) {
			return MutationResult[AdGroup]{}, invalidArgument(operation, "ad group name or CPC is invalid")
		}
		cpc := input.CPC
		operands = append(operands, AdGroup{
			CampaignID: campaignID, AdGroupName: input.Name, UserStatus: StatusPaused,
			Device: []DeviceType{DeviceNone}, DeviceApp: []DeviceAppType{DeviceAppNone},
			BiddingStrategyConfiguration: &AdGroupBiddingStrategy{BiddingScheme: &AdGroupBiddingScheme{
				CPC: &AdGroupCPCScheme{CPC: &cpc},
			}},
		})
	}
...
func (client *Client) UpdateAdGroups(ctx context.Context, campaignID int64, inputs []AdGroupUpdate, options ...socialhub.CallOption) (MutationResult[AdGroup], error) {
	...
	for _, input := range inputs {
		...
		if input.CPC != nil {
			cpc := *input.CPC
			operand.BiddingStrategyConfiguration = &AdGroupBiddingStrategy{BiddingScheme: &AdGroupBiddingScheme{
				CPC: &AdGroupCPCScheme{CPC: &cpc},
			}}
		}
...
func (client *Client) SetAdGroupsEnabled(ctx context.Context, campaignID int64, ids []int64, enabled bool, options ...socialhub.CallOption) (MutationResult[AdGroup], error) {
	...
	if campaignID <= 0 || !validIDs(ids, MaximumAdGroupMutationBatch, false) {
...
func (client *Client) requirePausedAdGroups(ctx context.Context, operation string, campaignID int64, ids []int64, options ...socialhub.CallOption) error {
	...
	for start := 0; start < len(ids); start += MaximumAdGroupSelectorIDs {
		end := start + MaximumAdGroupSelectorIDs
		if end > len(ids) {
			end = len(ids)
		}
		chunk := ids[start:end]
		...
		for _, adGroup := range page.Items {
			if adGroup.CampaignID != campaignID {
				return platformContractError(operation, "LINE Yahoo returned an ad group outside the requested campaign")
			}
...
func (client *Client) validateAdGroup(operation string, value *AdGroup, expectedCampaignID, expectedID int64) error {
	if value == nil || value.AccountID != client.advertiserAccountID || value.CampaignID <= 0 ||
		value.AdGroupID <= 0 || !validText(value.AdGroupName, 50) ||
		!validReturnedUserStatus(value.UserStatus) {
		return platformContractError(operation, "LINE Yahoo returned an invalid ad group")
	}
	if expectedCampaignID > 0 && value.CampaignID != expectedCampaignID || expectedID > 0 && value.AdGroupID != expectedID {
...
func (client *Client) validateAdGroupMutation(operation string, value *AdGroup, expectedCampaignID int64) error {
	if value == nil || value.AdGroupID <= 0 ||
