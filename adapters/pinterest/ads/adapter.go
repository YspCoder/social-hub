// Package ads implements ad-account-scoped Pinterest Ads API v5.28
...
const (
	adapterName      = "pinterest/ads-v5.28"
	...
	adsReadScope     = "ads:read"
	adsWriteScope    = "ads:write"
)

// Settings controls Pinterest API and OAuth endpoints. Overrides are intended
...
// Adapter implements socialhub.Adapter for Pinterest Ads API v5.28.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be ads")
	}
	...
	for _, endpoint := range []string{settings.BaseURL, settings.AuthURL, settings.TokenURL} {
		if !validEndpoint(endpoint) {
			return invalidArgument("init", "Pinterest endpoints must be absolute HTTP(S) URLs without credentials, query, or fragment")
		}
	}
	for _, account := range config.Accounts {
		if strings.TrimSpace(account.AccessTokenRef) == "" {
			return invalidArgument("init", "access_token_ref is required for every Pinterest Ads account")
		}
		...
		if !validID(typed.AdAccountID) {
			return invalidArgument("init", "account.settings.ad_account_id must be a numeric ID")
		}
...
// OAuth returns Pinterest's authorization-code, refresh-token, and
// client-credentials helper for the configured app.
