...
// ValidationDetail is one item in Unity's documented 400 response.
...
func decodeHTTPError(status int, header http.Header, body []byte) error {
	...
	if len(problem.Details) > 32 {
		problem.Details = problem.Details[:32]
	}
	for index := range problem.Details {
		problem.Details[index].Path = boundedMessage(problem.Details[index].Path, 256)
		problem.Details[index].Message = boundedMessage(redactSensitive(problem.Details[index].Message), 512)
		if len(problem.Details[index].Code) > 256 {
			problem.Details[index].Code = nil
		}
	}
	return &APIError{
