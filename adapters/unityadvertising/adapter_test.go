...
func TestAdapterRegistrationMetadataCapabilitiesAndLifecycle(t *testing.T) {
	...
	if adapter.Name() != adapterName || metadata.Product != productName || metadata.APIVersion != apiVersion || metadata.DocURL == "" || metadata.VerifiedAt.IsZero() || contractVersion != "v1.0 latest" {
		t.Fatalf("metadata=%#v", metadata)
	}
	capabilities, err := client.Capabilities(context.Background())
	if err != nil || !capabilities[CapabilityAdvertisingManagement].Supported || capabilities[CapabilityAdvertisingManagement].Approval != socialhub.ApprovalRequired {
	...
	if client.Apps() == nil || client.Creatives() == nil || client.CreativePacks() == nil || client.Campaigns() == nil || client.Bids() == nil ||
...
func TestAdapterConfigurationValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"bad organization", func(config *socialhub.AdapterConfig) {
...
func TestBasicAndBearerClientAuthentication(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		writeJSON(t, writer, http.StatusOK, Page[App]{Total: 1, Results: []App{appFixture()}})
	}))
	defer server.Close()
	adapter := &Adapter{}
	...
	if _, err := value.(*Client).ListApps(context.Background(), ListAppsRequest{}); err != nil {
		t.Fatal(err)
	}
}
