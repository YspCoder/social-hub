// Package unityadvertising implements Unity Advertising Management API v1 workflows.
...
const (
	adapterName      = "unity/advertising-management-api-v1"
	platformName     = "unity"
	productName      = "advertising-management-api"
	apiVersion       = "v1"
	contractVersion  = "v1.0 latest"
	defaultBaseURL   = "https://services.api.unity.com"
	documentationURL = "https://services.docs.unity.com/advertise/v1/"
)

// Settings controls the Advertising Management API endpoint. Overrides are
// intended for tests and controlled HTTP gateways.
...
// Adapter implements socialhub.Adapter for Unity Advertising Management API v1.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be advertising-management-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "webhook settings are not supported by Unity Advertising Management API v1")
		}
	}
