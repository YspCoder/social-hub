...
func TestAdapterRegistrationMetadataCapabilitiesAndLifecycle(t *testing.T) {
	...
	if client.Marketers() == nil || client.Budgets() == nil || client.Campaigns() == nil || client.PromotedLinks() == nil || client.Reports() == nil ||
...
func TestAdapterConfigurationValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"bad marketer", func(config *socialhub.AdapterConfig) { config.Accounts[0].Settings["marketer_id"] = "bad/id" }},
		{"missing credentials", func(config *socialhub.AdapterConfig) { config.Accounts[0].AccessTokenRef = "" }},
		{"static with login", func(config *socialhub.AdapterConfig) {
			config.Accounts[0].Settings["username"], config.Accounts[0].SecretRef = testUsername, "secret://password"
		}},
...
func TestAdapterClientAndLoginValidation(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {}))
	defer server.Close()
	adapter := &Adapter{}
	if _, err := adapter.Client(context.Background(), testAccountID); !errors.Is(err, socialhub.ErrConflict) {
	...
	if _, err := adapter.Login(context.Background(), testAccountID); !errors.Is(err, socialhub.ErrConflict) {
		t.Fatalf("uninitialized login error=%v", err)
	}
	adapter, _ = newTestAdapter(t, server)
	if _, err := adapter.Client(context.Background(), "missing"); !errors.Is(err, socialhub.ErrNotFound) {
		t.Fatalf("missing client error=%v", err)
	}
	if _, err := adapter.Login(context.Background(), "missing"); !errors.Is(err, socialhub.ErrNotFound) {
		t.Fatalf("missing login error=%v", err)
	}
	if _, err := adapter.Login(context.Background(), testAccountID); !errors.Is(err, socialhub.ErrInvalidArgument) {
		t.Fatalf("static-token login error=%v", err)
	}

	clock := &mutableClock{value: testNow}
	credentials := &Adapter{}
	if err := credentials.Init(context.Background(), loginConfig(server.URL),
	...
	login, err := credentials.Login(context.Background(), testAccountID)
	if err != nil || login.Username != testUsername || login.Password != testPassword || strings.HasSuffix(login.BaseURL, "/") || login.Clock != clock {
