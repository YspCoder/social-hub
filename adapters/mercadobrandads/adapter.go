// Package mercadobrandads implements Mercado Libre Brand Ads read and metrics workflows.
...
const (
	adapterName      = "mercadolibre/brand-ads-api"
	platformName     = "mercadolibre"
	productName      = "brand-ads-api"
	...
	documentationURL = "https://developers.mercadolibre.com.ar/en_us/ads-bads"
	approvalType     = "brand-ads-enabled"
)
...
// AccountSettings binds one SDK account to one Brand Ads advertiser. A zero
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be brand-ads-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "Mercado Libre notifications are outside this Brand Ads adapter")
		}
		...
		if !validApproval(account.Approval) {
			return invalidArgument("init", "approval must be omitted or declare brand-ads-enabled with the read scope")
		}
