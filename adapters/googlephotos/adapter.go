// Package googlephotos implements a bounded, app-created-only Google Photos Library API v1 adapter.
...
const (
	adapterName      = "google-photos/library-v1"
	platformName     = "google-photos"
	productName      = "photos-library-api"
	apiVersion       = "v1"
	defaultBaseURL   = "https://photoslibrary.googleapis.com"
	defaultUserAgent = "social-hub/googlephotos"
	documentationURL = "https://developers.google.com/photos/library/reference/rest"
	authorizationURL = "https://developers.google.com/photos/library/guides/authorization"
)

const (
	// ScopeReadAppCreatedData permits reads only for albums and media items
	// created by the same OAuth client application.
	ScopeReadAppCreatedData = "https://www.googleapis.com/auth/photoslibrary.readonly.appcreateddata"
	// ScopeAppendOnly is the current write-only scope for creating albums and
	// uploading media. Writes are outside this adapter.
	ScopeAppendOnly = "https://www.googleapis.com/auth/photoslibrary.appendonly"
	// ScopeEditAppCreatedData permits edits to app-created content. Edits are
	// outside this adapter.
	ScopeEditAppCreatedData = "https://www.googleapis.com/auth/photoslibrary.edit.appcreateddata"
)

// Settings contains adapter-wide request settings. The API origin is fixed to
// the official Google Photos host to prevent Bearer token redirection.
type Settings struct {
	UserAgent string `json:"user_agent,omitempty" yaml:"user_agent,omitempty"`
}

// Adapter implements socialhub.Adapter for Google Photos Library API v1.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be photos-library-api")
	}
	...
	for _, account := range config.Accounts {
		...
		for _, scope := range account.Approval.Scopes {
			if removedLibraryScope(scope) {
				return invalidArgument("init", "approval scopes contain a Google Photos Library scope removed after March 31, 2025")
			}
		}
