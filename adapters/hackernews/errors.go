...
func decodeHTTPError(status int, header http.Header, body []byte) error {
	...
	return &socialhub.Error{
		Code: code, Class: class, Platform: "hackernews", Product: productName, Op: "http",
		HTTPStatus: status, PlatformMessage: bounded(firstNonEmpty(envelope.Error, envelope.Message), 512),
...
func platformError(operation string, code socialhub.ErrorCode, class socialhub.ErrorClass, cause error) error {
	return &socialhub.Error{Code: code, Class: class, Platform: "hackernews", Product: productName, Op: operation, Cause: cause}
}

func invalidArgument(operation, message string) error {
	return &socialhub.Error{
		Code: socialhub.CodeInvalidArgument, Class: socialhub.ClassPermanent,
		Platform: "hackernews", Product: productName, Op: operation, PlatformMessage: message,
	}
}
...
func notFound(operation string) error {
	return &socialhub.Error{
		...
		Platform: "hackernews", Product: productName, Op: operation,
	}
}
