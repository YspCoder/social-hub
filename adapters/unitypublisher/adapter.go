// Package unitypublisher implements organization-bound Unity Ads Publisher Manage API v2 workflows.
// Monetization configuration remains separate from social-hub's organic interfaces.
...
const (
	adapterName      = "unity/ads-publisher-manage-api-v2"
	platformName     = "unity"
	productName      = "ads-publisher-manage-api"
	apiVersion       = "v2"
	contractVersion  = "1.0.0"
	defaultBaseURL   = "https://services.api.unity.com/ads/publisher"
	documentationURL = "https://docs.unity.com/en-us/oas-unified-platform-publisher-public/1.0.0"
)

// Settings controls the Publisher API endpoint. Overrides are intended for
...
// Adapter implements socialhub.Adapter for Unity Ads Publisher Manage API v2.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be ads-publisher-manage-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if !validOrganizationID(typed.OrganizationID) {
			return invalidArgument("init", "account.settings.organization_id must be a positive numeric Genesis organization ID")
		}
		...
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "webhook settings are not supported by Unity Ads Publisher Manage API v2")
		}
	}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	return &Client{accountID: accountID, organizationID: typed.OrganizationID, api: api}, nil
}
