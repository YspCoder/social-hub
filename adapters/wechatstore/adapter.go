// Package wechatstore implements selected read-only WeChat Store APIs.
package wechatstore
...
const (
	adapterName                 = "wechat/store-shop"
	platformName                = "wechat"
	productName                 = "store-shop"
	apiVersion                  = "continuous"
	baseURL                     = "https://api.weixin.qq.com"
	documentationURL            = "https://developers.weixin.qq.com/doc/store/shop/API/"
	developerGuideURL           = "https://developers.weixin.qq.com/doc/store/shop/dev_before/guide.html"
	stableTokenDocumentationURL = "https://developers.weixin.qq.com/doc/store/shop/API/apimgnt/common/api_getstableaccesstoken.html"
	storeInfoDocumentationURL   = "https://developers.weixin.qq.com/doc/store/shop/API/storemanage/api_mmecapi_basicinfo.html"
	productDocumentationURL     = "https://developers.weixin.qq.com/doc/store/shop/API/channels-shop-product/shop/api_getproduct.html"
)

// Adapter implements socialhub.Adapter for one or more self-managed stores.
type Adapter struct {
	mu      sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be store-shop")
	}
	...
	for _, account := range config.Accounts {
		...
		if account.Approval.AccountType != "" || len(account.Approval.Scopes) != 0 {
			return invalidArgument("init", "approval settings are not used for the self-managed store credential flow")
		}
