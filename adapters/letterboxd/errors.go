...
func decodeHTTPError(status int, header http.Header, body []byte) error {
	...
	case "invalid_grant", "invalid_token", "invalid_client":
	...
	return &socialhub.Error{
		Code: code, Class: class, Platform: "letterboxd", Product: productName, Op: "http",
		HTTPStatus: status, PlatformCode: bounded(firstNonEmpty(envelope.Error, envelope.Type), 128),
		PlatformMessage: bounded(firstNonEmpty(envelope.ErrorDescription, envelope.Message), 512),
...
func platformError(operation string, code socialhub.ErrorCode, class socialhub.ErrorClass, cause error) error {
	return &socialhub.Error{Code: code, Class: class, Platform: "letterboxd", Product: productName, Op: operation, Cause: cause}
}

func invalidArgument(operation, message string) error {
	return &socialhub.Error{
		Code: socialhub.CodeInvalidArgument, Class: socialhub.ClassPermanent,
		Platform: "letterboxd", Product: productName, Op: operation, PlatformMessage: message,
	}
}
