...
func TestOAuthAuthorizationExchangeAndRefresh(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "authorization_code":
			writeJSON(t, writer, http.StatusOK, map[string]any{
				...
				"scope": traffickingScope + " " + reportingScope, "token_type": "Bearer",
			})
		case "refresh_token":
	...
	query := parsed.Query()
	if query.Get("client_id") != "client-id" || query.Get("scope") != traffickingScope+" "+reportingScope ||
	...
	if err != nil || token.AccessToken != "access-2" || token.RefreshToken != "refresh-1" || token.TokenType != "Bearer" ||
		len(token.Scopes) != 2 || token.Scopes[0] != traffickingScope {
...
func TestManagedRefreshTokenSourceCachesAndRefreshes(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/userprofiles/" + testProfileID:
	...
	if _, err := client.GetProfile(context.Background()); err != nil || tokenCalls.Load() != 1 {
		t.Fatalf("cached request err=%v tokenCalls=%d", err, tokenCalls.Load())
	}
	clock.Set(testNow.Add(2 * time.Hour))
	if _, err := client.GetProfile(context.Background()); err != nil || tokenCalls.Load() != 2 {
...
func TestOAuthErrorsAndValidation(t *testing.T) {
	...
	if validCallbackURL("ftp://example.com/callback") || validCallbackURL("https://user:pass@example.com") ||
		!validCallbackURL("http://localhost/callback") || validOAuthScopes(nil) ||
		validOAuthScopes([]string{"openid"}) || validOAuthScopes([]string{reportingScope, reportingScope}) {
		t.Fatal("OAuth validation contract failed")
	}
	if boundedMessage(strings.Repeat("x", 30), 20) != strings.Repeat("x", 20) {
...
func TestRefreshTokenSourceStoreFailuresAndMemoryHit(t *testing.T) {
	...
	source = &refreshTokenSource{
		oauth: OAuthClient{
			...
			HTTPClient: http.DefaultClient, Clock: &mutableClock{value: testNow}, Scopes: []string{reportingScope},
		},
