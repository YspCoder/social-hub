...
func TestOAuthAuthorizationExchangeAndRefresh(t *testing.T) {
	...
	query := parsed.Query()
	if query.Get("client_id") != "client-id" || query.Get("scope") != readOnlyScope || query.Get("state") != "state-value" ||
...
func TestManagedRefreshTokenSourceCaches(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/v1beta/" + propertyName() + "/metadata":
			writeJSON(t, writer, http.StatusOK, map[string]any{"name": propertyName() + "/metadata"})
		default:
	...
	if _, err := client.GetMetadata(context.Background()); err != nil || tokenCalls.Load() != 1 {
		t.Fatalf("cached request err=%v tokenCalls=%d", err, tokenCalls.Load())
	}
	clock.Set(testNow.Add(2 * time.Hour))
	if _, err := client.GetMetadata(context.Background()); err != nil || tokenCalls.Load() != 2 {
		t.Fatalf("refreshed request err=%v tokenCalls=%d", err, tokenCalls.Load())
	}
}

func TestServiceAccountJWTAndCache(t *testing.T) {
	...
	defer server.Close()
	config := serviceAccountConfig(server.URL)
	adapter := &Adapter{}
	if err := adapter.Init(context.Background(), config,
	...
	secondCommon, err := adapter.Client(context.Background(), testAccountID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := secondCommon.(*Client).GetMetadata(context.Background()); err != nil || tokenCalls.Load() != 1 {
		t.Fatalf("store cache err=%v calls=%d", err, tokenCalls.Load())
	}
}
...
func verifyJWTAssertion(t *testing.T, assertion string, publicKey *rsa.PublicKey, audience string) {
	...
	if header["alg"] != "RS256" || claims["iss"] != "reports@test-project.iam.gserviceaccount.com" || claims["aud"] != audience || claims["scope"] != readOnlyScope ||
		int64(claims["exp"].(float64)-claims["iat"].(float64)) != int64(time.Hour/time.Second) {
