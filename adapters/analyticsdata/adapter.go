// Package analyticsdata implements property-scoped Google Analytics Data API v1beta workflows.
// Aggregated measurement data remains separate from organic social interfaces.
...
const (
	adapterName       = "google/analytics-data-api-v1beta"
	platformName      = "google-analytics"
	productName       = "analytics-data-api"
	apiVersion        = "v1beta"
	discoveryRevision = "20260804"
	defaultBaseURL    = "https://analyticsdata.googleapis.com"
	defaultAuthURL    = "https://accounts.google.com/o/oauth2/v2/auth"
	defaultTokenURL   = "https://oauth2.googleapis.com/token"
	documentationURL  = "https://developers.google.com/analytics/devguides/reporting/data/v1/rest"
	fullScope         = "https://www.googleapis.com/auth/analytics"
...
// AccountSettings binds one social-hub account to one GA4 property and selects
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be analytics-data-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if !validPropertyID(typed.PropertyID) {
			return invalidArgument("init", "account settings.property_id must be a positive string-encoded integer")
		}
		staticToken := strings.TrimSpace(account.AccessTokenRef) != ""
		managedOAuth := !staticToken && validOpaque(account.ClientID, 1024) && strings.TrimSpace(account.SecretRef) != "" && strings.TrimSpace(typed.RefreshTokenRef) != ""
		serviceAccount := !staticToken && validServiceAccountEmail(typed.ServiceAccountEmail) && strings.TrimSpace(typed.PrivateKeyRef) != ""
		modeCount := boolInt(staticToken) + boolInt(managedOAuth) + boolInt(serviceAccount)
		if modeCount != 1 {
		...
		if serviceAccount && (account.ClientID != "" || account.SecretRef != "" || typed.RefreshTokenRef != "") {
			return invalidArgument("init", "service-account authentication cannot be combined with user OAuth settings")
		}
		if !staticToken && len(oauthScopes(account.Approval.Scopes)) == 0 {
			return invalidArgument("init", "approval scopes must include a Google Analytics scope")
		}
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "webhook settings are not supported by Google Analytics Data API")
		}
	}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	return &Client{accountID: accountID, propertyID: typed.PropertyID, api: api, scopes: clientScopes}, nil
}
