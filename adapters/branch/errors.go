...
// newHTTPErrorDecoder deliberately drops Branch's free-form response body
// because it can echo the body credential or attribution identifiers.
func newHTTPErrorDecoder(clock socialhub.Clock) func(int, http.Header, []byte) error {
	return func(status int, header http.Header, _ []byte) error {
		...
		result := &socialhub.Error{
			...
			HTTPStatus: status, PlatformMessage: "Branch rejected the Events API request",
		...
		if code == socialhub.CodeUnauthenticated || code == socialhub.CodePermissionDenied {
			result.ApprovalURL = credentialsURL
		}
		return result
	}
}
...
func authenticationError(operation, message string) error {
	return &socialhub.Error{
		...
		PlatformMessage: message, ApprovalURL: credentialsURL,
	}
}
