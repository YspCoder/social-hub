...
func TestLinkedInOAuthFlows(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "authorization_code":
			writeJSON(writer, `{"access_token":"access","expires_in":3600,"refresh_token":"refresh","scope":"openid profile","token_type":"Bearer"}`)
		case "refresh_token":
			writeJSON(writer, `{"access_token":"renewed","expires_in":3600,"refresh_token":"refresh","scope":"openid profile"}`)
		default:
			writer.WriteHeader(http.StatusBadRequest)
		}
	}))
	...
	oauth, err := adapter.OAuth(context.Background(), "company")
	if err != nil {
		t.Fatal(err)
	}
	authorizationURL, err := oauth.AuthorizationURL("https://app.example/callback", "state", []string{"openid", "profile"})
	if err != nil {
		t.Fatal(err)
	}
	parsed, _ := url.Parse(authorizationURL)
	if parsed.Query().Get("scope") != "openid profile" || parsed.Query().Get("state") != "state" {
