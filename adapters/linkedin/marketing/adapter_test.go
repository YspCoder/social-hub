...
func TestRegistrationCapabilitiesAndLifecycle(t *testing.T) {
	...
	if adapter.Name() != adapterName || metadata.APIVersion != marketingVersion || metadata.Product != productName || metadata.DocURL == "" {
		t.Fatalf("metadata=%#v", metadata)
	}
	if client.Platform() != platformName || client.Account() != "b2b-demand" || client.adAccountID != testAdAccountID {
		t.Fatalf("client=%#v", client)
	}
	capabilities, err := client.Capabilities(context.Background())
	if err != nil || !capabilities.Has(CapabilityAdsManagement) || !capabilities.Has(CapabilityAdsReporting) {
	...
	if client.AdAccounts() == nil || client.CampaignGroups() == nil || client.Campaigns() == nil || client.Creatives() == nil || client.Analytics() == nil {
		t.Fatal("typed workflow missing")
	}
	if client.Close() != nil || adapter.Close() != nil || adapter.Close() != nil {
		t.Fatal("close failed")
	}
	if _, err := adapter.Client(context.Background(), "b2b-demand"); !errors.Is(err, socialhub.ErrConflict) {
		t.Fatalf("client after close=%v", err)
	}
	if _, err := adapter.OAuth(context.Background(), "b2b-demand"); !errors.Is(err, socialhub.ErrConflict) {
...
func TestAdapterValidationAndScopeGates(t *testing.T) {
	...
	config.Accounts[0].Approval.Scopes = []string{readAdsScope}
	adapter := &Adapter{}
	...
	if capabilities.Has(CapabilityAdsManagement) || capabilities.Has(CapabilityAdsReporting) {
		t.Fatalf("capabilities=%#v", capabilities)
	}
	_, err = client.CreateCampaign(context.Background(), validCampaignRequest())
	if !errors.Is(err, socialhub.ErrApprovalRequired) || hubError(t, err).RequiredScopes[0] != writeAdsScope {
		t.Fatalf("write scope=%v", err)
	}
	_, err = client.GetAdAnalytics(context.Background(), AnalyticsRequest{
		StartDate: "2026-08-01", EndDate: "2026-08-02", Pivot: PivotAccount,
		Granularity: GranularityDaily, Fields: []string{"impressions"},
	})
	if !errors.Is(err, socialhub.ErrApprovalRequired) {
		t.Fatalf("reporting scope=%v", err)
	}
