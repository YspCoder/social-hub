...
func decodeHTTPError(status int, header http.Header, body []byte) error {
	...
	message := firstNonEmpty(response.Message, response.ErrorDescription)
	code, class := classifyError(status, message)
	platformCode := firstNonEmpty(response.Code, response.Error)
...
func classifyError(status int, message string) (socialhub.ErrorCode, socialhub.ErrorClass) {
	upper := strings.ToUpper(message)
	if strings.Contains(upper, "RATE LIMIT") || strings.Contains(upper, "THROTTL") || strings.Contains(upper, "QUOTA") {
