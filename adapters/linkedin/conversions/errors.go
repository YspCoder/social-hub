...
type errorEnvelope struct {
	ServiceErrorCode *int64 `json:"serviceErrorCode"`
}

// LinkedIn validation messages can contain batch indexes and rejected PII, so
// only numeric codes and response metadata leave this decoder.
func newHTTPErrorDecoder(clock socialhub.Clock) func(int, http.Header, []byte) error {
	return func(status int, header http.Header, body []byte) error {
		...
		platformCode := ""
		if envelope.ServiceErrorCode != nil {
			platformCode = strconv.FormatInt(*envelope.ServiceErrorCode, 10)
		}
		code, class := classifyError(status)
		requestID := firstNonEmpty(header.Get("x-li-uuid"), header.Get("x-request-id"), header.Get("x-restli-id"))
		if !validOptionalOpaque(requestID, 256) {
			requestID = ""
		}
		hub := &socialhub.Error{
			Code: code, Class: class, Platform: platformName, Product: productName, HTTPStatus: status,
			PlatformCode: platformCode, PlatformMessage: "LinkedIn rejected the conversion request",
...
func redactCredentialMessage(value, credential string) string {
	...
	for _, key := range []string{"access_token", "authorization", "bearer", "client_secret", "password"} {
		cursor := 0
		for {
