...
func TestAdapterRegistrationMetadataCapabilitiesAndClose(t *testing.T) {
	...
	if err != nil || !capabilities[CapabilityYouTubeAnalytics].Supported || capabilities[socialhub.CapFetch].Supported {
		t.Fatalf("capabilities=%#v err=%v", capabilities, err)
	}
	if client.Platform() != platformName || client.Account() != testAccountID || client.Reporting() != client || client.Groups() != client {
	...
	if client.reportIDs() != "channel=="+testChannelID || client.Close() != nil || adapter.Close() != nil {
		t.Fatal("binding or close failed")
	}
...
func TestAdapterInitValidation(t *testing.T) {
	...
	tests := []func(socialhub.AdapterConfig) socialhub.AdapterConfig{
		...
		func(value socialhub.AdapterConfig) socialhub.AdapterConfig {
			value.Accounts[0].Settings["content_owner_id"] = testOwnerID
			return value
		},
		func(value socialhub.AdapterConfig) socialhub.AdapterConfig {
			delete(value.Accounts[0].Settings, "channel_id")
			return value
		},
		func(value socialhub.AdapterConfig) socialhub.AdapterConfig {
			value.Accounts[0].Settings["channel_id"] = "bad/id"
			return value
		},
		func(value socialhub.AdapterConfig) socialhub.AdapterConfig {
...
func TestAdapterClientSecretsOverridesAndScopes(t *testing.T) {
	server := httptest.NewServer(http.NotFoundHandler())
	defer server.Close()
	adapter := &Adapter{}
	...
	config.Accounts[0].Approval.Scopes = []string{analyticsReadScope}
	_, client := newStaticClient(t, scopeServer, config)
	if _, err := client.QueryReport(context.Background(), reportRequest()); !errors.Is(err, socialhub.ErrApprovalRequired) || requestSeen {
		t.Fatalf("report scope error=%v requestSeen=%v", err, requestSeen)
	}
	if _, err := client.ListGroups(context.Background(), ListGroupsRequest{Mine: true}); !errors.Is(err, socialhub.ErrApprovalRequired) || requestSeen {
		t.Fatalf("read scope error=%v requestSeen=%v", err, requestSeen)
	}

	config = staticConfig(scopeServer.URL)
	config.Accounts[0].Approval.Scopes = []string{youtubeReadOnlyScope, analyticsReadScope}
	_, client = newStaticClient(t, scopeServer, config)
	if _, err := client.CreateGroup(context.Background(), CreateGroupInput{Title: "Group", ItemType: ResourceVideo}); !errors.Is(err, socialhub.ErrApprovalRequired) || requestSeen {
		t.Fatalf("write scope error=%v requestSeen=%v", err, requestSeen)
	}

	config = ownerConfig(scopeServer.URL)
	config.Accounts[0].Approval.Scopes = []string{youtubeScope, analyticsReadScope}
	_, client = newStaticClient(t, scopeServer, config)
	if err := client.DeleteGroup(context.Background(), "group1"); !errors.Is(err, socialhub.ErrApprovalRequired) || requestSeen {
		t.Fatalf("owner write scope error=%v requestSeen=%v", err, requestSeen)
	}
}
...
func TestClientRejectsCredentialRedirect(t *testing.T) {
	...
	if _, err := client.QueryReport(context.Background(), reportRequest()); err == nil || redirected {
