...
// flow with offline access. YouTube Analytics does not support service accounts.
...
func (client *OAuthClient) AuthorizationURL(redirectURI, state string) (string, error) {
	if !validOpaque(client.ClientID, 1024) || !validCallbackURL(redirectURI) || !validOpaque(state, 1024) ||
		...
		return "", invalidArgument("oauth_authorize", "client ID, redirect URI, state, authorization endpoint, or YouTube scopes are invalid")
	}
