...
func TestOAuthAuthorizationExchangeAndRefresh(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "authorization_code":
			...
			writeJSON(t, writer, http.StatusOK, map[string]any{
				...
				"scope": youtubeReadOnlyScope + " " + analyticsReadScope, "token_type": "Bearer",
			})
		case "refresh_token":
	...
	query := parsed.Query()
	if query.Get("client_id") != "client-id" || query.Get("scope") != youtubeReadOnlyScope+" "+analyticsReadScope ||
...
func TestManagedRefreshTokenSourceCaches(t *testing.T) {
	...
	if _, err := client.QueryReport(context.Background(), reportRequest()); err != nil {
		t.Fatal(err)
	}
	if _, err := client.QueryReport(context.Background(), reportRequest()); err != nil || tokenCalls.Load() != 1 {
		t.Fatalf("cached request err=%v tokenCalls=%d", err, tokenCalls.Load())
	}
	clock.Set(testNow.Add(2 * time.Hour))
	if _, err := client.QueryReport(context.Background(), reportRequest()); err != nil || tokenCalls.Load() != 2 {
		t.Fatalf("refreshed request err=%v tokenCalls=%d", err, tokenCalls.Load())
	}
}

func TestOAuthAndCredentialErrors(t *testing.T) {
	tests := []struct {
		status int
		body   string
	...
	for _, test := range tests {
		...
		oauth := &OAuthClient{
			ClientID: "client-id", ClientSecret: "client-secret", AuthURL: server.URL + "/authorize", TokenURL: server.URL,
			HTTPClient: server.Client(), Clock: &mutableClock{value: testNow}, Scopes: []string{youtubeReadOnlyScope, analyticsReadScope},
		}
	...
	client = &OAuthClient{ClientID: "id", ClientSecret: "secret", AuthURL: "https://example.com", TokenURL: "https://example.com", HTTPClient: http.DefaultClient, Clock: &mutableClock{value: testNow}, Scopes: []string{"bad"}}
	if _, err := client.AuthorizationURL("https://app.example/callback", "state"); !errors.Is(err, socialhub.ErrInvalidArgument) {
...
func TestTokenSourceStoreFailuresAndMemoryHit(t *testing.T) {
	source := &refreshTokenSource{oauth: OAuthClient{Clock: &mutableClock{value: testNow}}, store: failingTokenStore{getErr: errors.New("store unavailable")}}
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, _ *http.Request) {
		writeJSON(t, writer, http.StatusOK, map[string]any{"access_token": "new", "expires_in": 3600, "token_type": "Bearer"})
	}))
	defer server.Close()
	source = &refreshTokenSource{
		oauth:        OAuthClient{ClientID: "id", ClientSecret: "secret", TokenURL: server.URL, HTTPClient: server.Client(), Clock: &mutableClock{value: testNow}, Scopes: []string{youtubeScope}},
		refreshToken: "refresh", store: failingTokenStore{getErr: socialhub.ErrNotFound, putErr: errors.New("put unavailable")},
	}
	if _, err := source.Token(context.Background()); !errors.Is(err, socialhub.ErrUnavailable) {
...
func TestOAuthRedirectClientDoesNotFollow(t *testing.T) {
	followed := false
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.URL.Path == "/token" {
			http.Redirect(writer, request, "/target?access_token=secret", http.StatusFound)
			return
		}
		followed = true
	}))
	...
	if err := adapter.Init(context.Background(), config, socialhub.WithHTTPClient(server.Client()),
		socialhub.WithSecretResolver(mapResolver{"test://client-secret": "secret", "test://refresh-token": "refresh"})); err != nil {
		t.Fatal(err)
	}
	common, err := adapter.Client(context.Background(), testAccountID)
	if err != nil {
	...
	_, err = common.(*Client).QueryReport(context.Background(), reportRequest())
	if err == nil || followed {
