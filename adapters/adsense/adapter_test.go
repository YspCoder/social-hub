...
func TestAdapterRegistrationMetadataCapabilitiesAndClose(t *testing.T) {
	...
	if err != nil || !capabilities[CapabilityAdSense].Supported || capabilities[socialhub.CapFetch].Supported {
		t.Fatalf("capabilities=%#v err=%v", capabilities, err)
	}
	if client.Platform() != platformName || client.Account() != testAccountID || client.Accounts() != client ||
		client.Inventory() != client || client.Compliance() != client || client.Reporting() != client {
...
func TestAdapterClientAndScopeErrors(t *testing.T) {
	server := httptest.NewServer(http.NotFoundHandler())
	defer server.Close()
	adapter := &Adapter{}
	...
	if _, err := common.(*Client).GetAccount(context.Background()); !errors.Is(err, socialhub.ErrApprovalRequired) || requestSeen {
...
func TestRedirectsAreRejected(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		if request.URL.Path == "/v2/"+accountName() {
