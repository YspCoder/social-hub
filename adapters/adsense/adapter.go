// Package adsense implements account-bound Google AdSense Management API v2 workflows.
// AdSense inventory and earnings remain separate from organic social interfaces.
package adsense
...
const (
	adapterName       = "google/adsense-management-api-v2"
	platformName      = "google-adsense"
	productName       = "adsense-management-api"
	apiVersion        = "v2"
	discoveryRevision = "20260806"
	defaultBaseURL    = "https://adsense.googleapis.com"
	defaultAuthURL    = "https://accounts.google.com/o/oauth2/v2/auth"
	defaultTokenURL   = "https://oauth2.googleapis.com/token"
	documentationURL  = "https://developers.google.com/adsense/management/reference/rest"
	fullScope         = "https://www.googleapis.com/auth/adsense"
	readOnlyScope     = "https://www.googleapis.com/auth/adsense.readonly"
)
...
// AccountSettings binds one social-hub account to one AdSense publisher account.
...
// Adapter implements socialhub.Adapter for AdSense Management API v2.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be adsense-management-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if !validPublisherID(typed.PublisherID) {
			return invalidArgument("init", "account settings.publisher_id must match pub-[0-9]+")
		}
		...
		if !staticToken && !managedOAuth {
			return invalidArgument("init", "configure access_token_ref or client_id, secret_ref, and account.settings.refresh_token_ref; AdSense does not support service accounts")
		}
		if staticToken {
		...
		if managedOAuth && len(oauthScopes(account.Approval.Scopes)) == 0 {
			return invalidArgument("init", "approval scopes must include an AdSense Management API scope")
		}
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "webhook settings are not supported by AdSense Management API")
		}
	}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	return &Client{accountID: accountID, publisherID: typed.PublisherID, api: api, scopes: clientScopes}, nil
}
