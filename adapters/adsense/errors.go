...
// APIError augments socialhub.Error with a bounded, sanitized Google status.
...
func (err *APIError) Error() string {
	if err == nil || err.Hub == nil {
		return "socialhub: google-adsense: platform_error"
	}
...
func decodeHTTPError(status int, header http.Header, body []byte) error {
	...
	if len(google.Details) > 32 {
		google.Details = append([]GoogleErrorDetail(nil), google.Details[:32]...)
	}
	reason := ""
	for index := range google.Details {
		google.Details[index].Type = boundedMessage(redactSensitive(google.Details[index].Type), 256)
		google.Details[index].Reason = boundedMessage(redactSensitive(google.Details[index].Reason), 256)
		google.Details[index].Domain = boundedMessage(redactSensitive(google.Details[index].Domain), 256)
		if reason == "" {
			reason = google.Details[index].Reason
		}
	}
...
func classifyError(status int, platformStatus, reason string) (socialhub.ErrorCode, socialhub.ErrorClass) {
	...
	switch normalized {
	case "ratelimitexceeded", "quotaexceeded", "exceededquota", "resourceexhausted", "reportrowlimitexceeded":
...
func ownershipError(operation, resource string) error {
	return &socialhub.Error{Code: socialhub.CodePermissionDenied, Class: socialhub.ClassUserAction, Platform: platformName, Product: productName, Op: operation, PlatformMessage: resource + " is not owned by the configured publisher account"}
}
