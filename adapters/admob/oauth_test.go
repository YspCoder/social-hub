...
func TestOAuthAuthorizationExchangeAndRefresh(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "authorization_code":
			writeJSON(t, writer, http.StatusOK, map[string]any{"access_token": "access-1", "expires_in": 3600, "refresh_token": "refresh-1", "scope": readOnlyScope, "token_type": "Bearer"})
		case "refresh_token":
...
func TestOAuthErrorsAndValidation(t *testing.T) {
	...
	if validCallbackURL("ftp://example.com/callback") || validCallbackURL("https://user:pass@example.com") ||
		...
		validOAuthScopes([]string{readOnlyScope, readOnlyScope}) || boundedMessage(strings.Repeat("界", 30), 20) != strings.Repeat("界", 20) {
...
func TestRefreshTokenSourceStoreFailuresAndMemoryHit(t *testing.T) {
	...
	source = &refreshTokenSource{
		oauth:        OAuthClient{ClientID: "client", ClientSecret: "secret", TokenURL: "https://example.com/token", HTTPClient: http.DefaultClient, Clock: &mutableClock{value: testNow}, Scopes: []string{readOnlyScope}},
