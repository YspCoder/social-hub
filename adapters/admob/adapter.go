// Package admob implements publisher-bound Google AdMob API v1 workflows.
// AdMob inventory and monetization reports remain separate from organic social interfaces.
package admob
...
const (
	adapterName       = "google/admob-api-v1"
	platformName      = "google-admob"
	productName       = "admob-api"
	apiVersion        = "v1"
	discoveryRevision = "20260731"
	defaultBaseURL    = "https://admob.googleapis.com"
	defaultAuthURL    = "https://accounts.google.com/o/oauth2/v2/auth"
	defaultTokenURL   = "https://oauth2.googleapis.com/token"
	documentationURL  = "https://developers.google.com/admob/api/reference/rest"
	readOnlyScope     = "https://www.googleapis.com/auth/admob.readonly"
	reportScope       = "https://www.googleapis.com/auth/admob.report"
)
...
// AccountSettings binds one social-hub account to one AdMob publisher account.
...
// Adapter implements socialhub.Adapter for AdMob API v1.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	if config.Product != "" && config.Product != productName {
		return invalidArgument("init", "product must be admob-api")
	}
	...
	for _, account := range config.Accounts {
		...
		if !staticToken && !managedOAuth {
			return invalidArgument("init", "configure access_token_ref or client_id, secret_ref, and account.settings.refresh_token_ref; AdMob requires user OAuth")
		}
		if staticToken {
		...
		if managedOAuth && len(oauthScopes(account.Approval.Scopes)) == 0 {
			return invalidArgument("init", "approval scopes must include an AdMob API scope")
		}
		if account.Webhook != (socialhub.WebhookConfig{}) {
			return invalidArgument("init", "webhook settings are not supported by AdMob API v1")
		}
	}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	return &Client{accountID: accountID, publisherID: typed.PublisherID, api: api, httpClient: httpClient, scopes: clientScopes}, nil
}
...
func oauthScopes(configured []string) []string {
	...
	for _, candidate := range []string{readOnlyScope, reportScope} {
