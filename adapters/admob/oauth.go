...
// with offline access and refresh tokens. AdMob authorization represents a user.
...
func (client *OAuthClient) AuthorizationURL(redirectURI, state string) (string, error) {
	if !validOpaque(client.ClientID, 1024) || !validCallbackURL(redirectURI) ||
		...
		return "", invalidArgument("oauth_authorize", "client ID, redirect URI, state, authorization endpoint, or AdMob scopes are invalid")
	}
