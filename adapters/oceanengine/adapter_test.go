...
func TestAdapterMetadataCapabilitiesAndLifecycle(t *testing.T) {
	...
	if err != nil || !capabilities.Has(CapabilityMarketingManagement) || !capabilities.Has(CapabilityMarketingReports) {
		t.Fatalf("capabilities=%#v err=%v", capabilities, err)
	}
	if capabilities.Has(socialhub.CapPublish) {
		t.Fatal("organic publish must be unsupported")
	}
	if client.Platform() != platformName || client.Account() != "ads-primary" || client.Projects() == nil || client.Promotions() == nil || client.Reports() == nil {
...
func TestAdapterValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"endpoint", func(config *socialhub.AdapterConfig) { config.Settings["base_url"] = "https://user:pass@example.com" }},
