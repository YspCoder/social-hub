// Package patreon implements the official Patreon API v2.
package patreon
...
const (
	adapterName      = "patreon/api-v2"
	productName      = "api"
	apiVersion       = "v2"
	defaultBaseURL   = "https://www.patreon.com/api/oauth2/v2"
	defaultAuthURL   = "https://www.patreon.com/oauth2/authorize"
	defaultTokenURL  = "https://www.patreon.com/api/oauth2/token"
	documentationURL = "https://docs.patreon.com/"
)

// Settings controls Patreon API and OAuth endpoints.
...
// AccountSettings selects the creator campaign used by common workflows.
type AccountSettings struct {
	CampaignID string `json:"campaign_id" yaml:"campaign_id"`
	UserID     string `json:"user_id,omitempty" yaml:"user_id,omitempty"`
}

// Adapter implements socialhub.Adapter for Patreon API v2.
type Adapter struct {
	mu       sync.RWMutex
...
func (adapter *Adapter) Init(_ context.Context, config socialhub.AdapterConfig, options ...socialhub.Option) error {
	...
	for _, endpoint := range []string{settings.BaseURL, settings.AuthURL, settings.TokenURL} {
		if !validEndpoint(endpoint) {
			return invalidArgument("init", "all Patreon endpoints must be absolute HTTP(S) URLs without credentials, query, or fragment")
		}
	}
	for _, account := range config.Accounts {
		var typed AccountSettings
		if err := socialhub.DecodeSettings(account.Settings, &typed); err != nil || !validResourceID(typed.CampaignID) || typed.UserID != "" && !validResourceID(typed.UserID) {
			return invalidArgument("init", "account.settings.campaign_id is required and user_id must be a valid resource ID when set")
		}
	}
...
func (adapter *Adapter) Client(ctx context.Context, accountID socialhub.AccountID, options ...socialhub.Option) (socialhub.Client, error) {
	...
	if strings.TrimSpace(account.AccessTokenRef) != "" {
		...
		api, err = transport.New(settings.BaseURL, &httpClient, tokens, "patreon", productName, decodeHTTPError)
		if err != nil {
			return nil, platformError("client", socialhub.CodeInvalidArgument, socialhub.ClassPermanent, err)
		}
	}
	...
	return &Client{
		accountID: accountID, campaignID: typed.CampaignID, userID: typed.UserID,
		api: api, scopes: append([]string(nil), account.Approval.Scopes...), webhookSecret: webhookSecret,
		clock: resolved.Clock,
	}, nil
}

// OAuth returns a Patreon API v2 authorization-code and refresh helper.
